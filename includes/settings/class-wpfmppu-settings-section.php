...
if ( ! class_exists( 'WPFMPPU_Settings_Section' ) ) :

class WPFMPPU_Settings_Section {
	...
	function __construct() {
		add_filter(
			'woocommerce_get_sections_' . 'wpfmppu',
			array( $this, 'settings_section' )
		);
		add_filter(
			'woocommerce_get_settings_' . 'wpfmppu' . '_' . $this->id,
	...
	function get_section_link( $section, $title = '' ) {
		...
		return '<a href="' . admin_url( 'admin.php?page=wc-settings&tab=wpfmppu&section=' . $section_id ) . '">' . $title . '</a>';
	}
	...
	function get_placeholders_desc( $values, $template = false ) {
		return sprintf(
			...
			( false !== $template ? $template : __( 'Placeholders: %s.', 'maximum-products-per-user-for-woocommerce' ) ),
	...
	function get_recalculate_sales_data_desc( $option ) {
		return sprintf(
			/* Translators: %1$s: Link, %2$s: Option name. */
			__( 'You will need to %1$s after changing "%2$s" option.', 'maximum-products-per-user-for-woocommerce' ),
