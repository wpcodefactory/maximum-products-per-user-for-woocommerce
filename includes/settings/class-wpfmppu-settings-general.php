...
if ( ! class_exists( 'WPFMPPU_Settings_General' ) ) :

class WPFMPPU_Settings_General extends WPFMPPU_Settings_Section {
	...
	function get_settings() {
		...
		$general_settings = array(
			array(
				...
				'desc'     => '<p>' . '* ' .
				              sprintf(
				                /* Translators: %s: Link. */
								__( 'While data is recalculated automatically (but only after the plugin was enabled), you can also force manual recalculation by running %s tool.', 'maximum-products-per-user-for-woocommerce' ),
			...
			array(
				...
				'type'     => 'multiselect',
				'class'    => 'chosen_select',
				'options'  => wpfmppu()->core->get_user_roles( false ),
			),
			array(
		...
		$date_range_options = array(
			...
			array(
				...
				'desc'     => sprintf(
					/* Translators: %s: Date range unit. */
					__( 'In %s.', 'maximum-products-per-user-for-woocommerce' ),
					get_option( 'alg_wc_mppu_date_range_custom_unit', 'seconds' )
				),
			...
			array(
				...
				'desc_tip' => sprintf(
					...
					__( 'Used for the "%s" option.', 'maximum-products-per-user-for-woocommerce' ),
					__( 'Custom date range', 'maximum-products-per-user-for-woocommerce' )
				),
			...
			array(
				...
				'desc_tip' => sprintf(
					...
					__( 'Used with the "%s" date range option.', 'maximum-products-per-user-for-woocommerce' ),
					__( 'Fixed date', 'maximum-products-per-user-for-woocommerce' )
				),
			...
			array(
				...
				'desc_tip' => sprintf(
					/* Translators: %s: Option value. */
					__( 'Used with the "%s" date range option.', 'maximum-products-per-user-for-woocommerce' ),
					__( 'Monthly', 'maximum-products-per-user-for-woocommerce' )
				) . ' ' .
				sprintf(
					/* Translators: %1$s: Option value, %2$s: Date example. */
					__( 'If the user register date is %1$s, the limits would be reset monthly, like: %2$s and so on, always on day 15th.', 'maximum-products-per-user-for-woocommerce' ),
					'<code>2023-04-15</code>',
			...
			array(
				...
				'desc_tip' => sprintf(
					/* Translators: %s: Option value. */
					__( 'Used with the "%s" date range option.', 'maximum-products-per-user-for-woocommerce' ),
					__( 'This week', 'maximum-products-per-user-for-woocommerce' )
	              ) . ' ' .
	              sprintf(
					  /* Translators: %s: Option name. */
					  __( 'Should be used along with the "%s" option.', 'maximum-products-per-user-for-woocommerce' ),
					  __( 'Advanced > Current time offset', 'maximum-products-per-user-for-woocommerce' )
	              ),
				'id'       => 'alg_wc_mppu_date_range_week_starts_on',
				'default'  => wpfmppu()->core->weekdays ? wpfmppu()->core->weekdays->get_week_starts_on_default_val() : '0',
				'options'  => wpfmppu()->core->weekdays ? wpfmppu()->core->weekdays->get_week_days_by_key_and_value( 'name_formatted' ) : array(),
				'type'     => 'select',
			),
			array(
				'type'     => 'sectionend',
...
return new WPFMPPU_Settings_General();
