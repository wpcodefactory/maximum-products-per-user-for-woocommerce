...
if ( ! class_exists( 'WPFMPPU_Reports' ) ) :

class WPFMPPU_Reports {
	...
	function product_terms_show_data( $term, $taxonomy ) {
		if ( 'yes' !== wpfmppu_get_option( 'alg_wc_mppu_enable_term_sales_data', 'yes' ) ) {
			return;
		}

		echo '<h2>' . esc_html__( 'Maximum Products per User: Sales Data', 'maximum-products-per-user-for-woocommerce' ) . '</h2>';

		if ( 'no' === wpfmppu_get_option( 'alg_wc_mppu_enable_term_sales_data_via_ajax', 'yes' ) ) {
			echo wp_kses_post( $this->get_report_data_table( $term->term_id, false ) );
		} else {
		...
		echo wp_kses( $this->get_admin_product_page_styles(), wpfmppu()->core->get_allowed_style() );
	}
	...
	function add_report_meta_box() {
		if ( 'yes' === wpfmppu_get_option( 'alg_wc_mppu_enable_product_sales_data', 'yes' ) ) {
			add_meta_box(
	...
	function render_product_sales_data_meta_box() {
		...
		if ( 'no' === wpfmppu_get_option( 'alg_wc_mppu_enable_product_sales_data_via_ajax', 'yes' ) ) {
			echo wp_kses_post( $this->get_product_sales_data_html( $product_id ) );
		} else {
			$sales_data_btn      = new WPFMPPU_Sales_Data_Btn();
			$sales_data_btn_html = $sales_data_btn->get_btn_html( array(
				'style'      => 'margin-top:5px',
	...
	function get_report_data_table( $product_or_term_id, $is_product ) {
		...
		if ( $is_product ) {
			$html.= sprintf(
				...
				'<h3>' . __( 'Product #%d', 'maximum-products-per-user-for-woocommerce' ) . '</h3>',
				$product_or_term_id
			);
		}
		// Lifetime
		$users_quantities = wpfmppu()->core->get_post_or_term_meta( ( $is_product ? 'product' : 'term' ), $product_or_term_id, '_alg_wc_mppu_totals_data' );
		...
		// Orders
		$users_orders_quantities = wpfmppu()->core->get_post_or_term_meta( ( $is_product ? 'product' : 'term' ), $product_or_term_id, '_alg_wc_mppu_orders_data' );
