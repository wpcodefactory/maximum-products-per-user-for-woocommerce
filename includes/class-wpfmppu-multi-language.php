...
if ( ! class_exists( 'WPFMPPU_Multi_Language' ) ) :

class WPFMPPU_Multi_Language {
	...
	function __construct() {
		if ( 'no' != ( $multi_language = get_option( 'alg_wc_mppu_multi_language', 'no' ) ) ) {
			add_filter( 'wpfmppu_data_product_or_term_id', array( $this, $multi_language ), 10, 2 );
		}
	}
	...
	function wpml( $product_or_term_id, $is_product ) {
		...
		if ( $this->default_language ) {
			...
			if ( isset( $element_type ) && $element_type ) {
				return apply_filters( 'wpml_object_id', $product_or_term_id, $element_type, true, $this->default_language ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
			}
		}
		// Return original ID
...
return new WPFMPPU_Multi_Language();
