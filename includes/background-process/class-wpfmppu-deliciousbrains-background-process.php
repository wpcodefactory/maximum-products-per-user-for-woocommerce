...
/**
 * Abstract WPFMPPU_DeliciousBrains_Background_Process class.
 *
 * @abstract
 * @extends WPFMPPU_DeliciousBrains_Async_Request
 */
abstract class WPFMPPU_DeliciousBrains_Background_Process extends WPFMPPU_DeliciousBrains_Async_Request {
	/**
	...
	public function __construct( $allowed_batch_data_classes = true ) {
		...
		add_filter( $this->identifier . '_query_args', array( $this, 'filter_dispatch_query_args' ) ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.DynamicHooknameFound
	}
	...
	public function dispatch() {
		 ...
		 */
		$cancel = apply_filters( $this->identifier . '_pre_dispatch', false, $this->get_chain_id() ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.DynamicHooknameFound
	...
	protected function cancelled() {
		do_action( $this->identifier . '_cancelled', $this->get_chain_id() ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.DynamicHooknameFound
	}
	...
	protected function paused() {
		do_action( $this->identifier . '_paused', $this->get_chain_id() ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.DynamicHooknameFound
	}
	...
	protected function resumed() {
		do_action( $this->identifier . '_resumed', $this->get_chain_id() ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.DynamicHooknameFound
	}
	...
	public function lock_process( $reset_start_time = true ) {
		 ...
		 */
		do_action(
			$this->identifier . '_process_locked', // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.DynamicHooknameFound
			$locked,
			$microtime,
			$lock_duration,
			$this->get_chain_id()
	...
	protected function unlock_process() {
		 ...
		 * @param string $chain_id Current background process chain ID.
		 */
		do_action( $this->identifier . '_process_unlocked', $unlocked, $this->get_chain_id() ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.DynamicHooknameFound
	...
	public function get_batches( $limit = 0 ) {
		...
		$items = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter
			$wpdb->prepare(
	...
	protected function handle() {
		...
		$throttle_seconds = max(
			0,
			apply_filters(
				$this->identifier . '_seconds_between_batches', // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.DynamicHooknameFound
				apply_filters(
					$this->prefix . '_seconds_between_batches', // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.DynamicHooknameFound
					0
				)
			)
		);
	...
	protected function completed() {
		do_action( $this->identifier . '_completed', $this->get_chain_id() ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.DynamicHooknameFound
	}
	...
	public function get_cron_interval() {
		...
		$interval = apply_filters( $this->cron_interval_identifier, $interval ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.DynamicHooknameFound
	...
	public function schedule_cron_healthcheck( $schedules ) {
		...
		if ( 1 === $interval ) {
			$display = __( 'Every Minute', 'maximum-products-per-user-for-woocommerce' );
		} else {
			$display = sprintf(
				/* Translators: %d: Minutes. */
				__( 'Every %d Minutes', 'maximum-products-per-user-for-woocommerce' ),
				$interval
			);
		}
	...
	public function should_continue() {
		...
		return apply_filters(
			$this->identifier . '_should_continue', // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.DynamicHooknameFound
	...
	private function get_chain_id_arg_name() {
		 ...
		 * @return string
		 */
		$chain_id_arg_name = apply_filters( $this->identifier . '_chain_id_arg_name', self::CHAIN_ID_ARG_NAME ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.DynamicHooknameFound
