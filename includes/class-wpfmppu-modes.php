...
if ( ! class_exists( 'WPFMPPU_Modes' ) ) :

	class WPFMPPU_Modes extends WPFMPPU_Dynamic_Properties_Obj {
		...
		function __construct() {
			...
			add_filter( 'wpfmppu_get_cart_item_quantities', array( $this, 'get_cart_item_quantities_by_mode' ) );
			add_filter( 'wpfmppu_validate_on_add_to_cart_quantity', array( $this, 'validate_on_add_to_cart_quantity_by_mode' ), 10, 2 );
			add_filter( 'wpfmppu_save_quantities_item_qty', array( $this, 'save_quantities_item_qty_by_mode' ), 10, 3 );
			if ( 'orders' === $this->mode ) {
				add_filter( 'wpfmppu_get_cart_item_amount_by_term', array( $this, 'cart_item_amount' ) );
				add_filter( 'wpfmppu_get_cart_item_amount_by_parent', array( $this, 'cart_item_amount' ) );
				add_filter( 'wpfmppu_cart_item_amount', array( $this, 'cart_item_amount' ) );
				add_filter( 'wpfmppu_validate_on_add_to_cart_quantity_do_add', array( $this, 'validate_on_add_to_cart_quantity_do_add' ) );
				add_filter( 'wpfmppu_orders_data_increase_qty', array( $this, 'handle_orders_data_quantity_increase_qty_on_order_mode' ), 10, 5 );
				add_filter( 'wpfmppu_totals_data', array( $this, 'handle_totals_data_on_order_mode' ), 10, 5 );
			}
		}
		...
		function get_cart_item_quantities_by_mode( $cart_item_quantities ) {
			foreach ( WC()->cart->get_cart() as $item ) {
				...
				switch ( $this->mode ) {
					case 'price':
						$cart_item_quantities[ $product_id ] = round( ( $item['line_total'] + $item['line_tax'] ), wc_get_price_decimals() );
						break;
					case 'price_excl_tax':
						$cart_item_quantities[ $product_id ] = round( $item['line_total'], wc_get_price_decimals() );
						break;
					case 'weight':
						$product = wc_get_product( $product_id );
						$weight  = ( $product && ( $weight = $product->get_weight() ) ? $weight : 0 );
						$cart_item_quantities[ $product_id ] = $item['quantity'] * $weight;
						break;
					case 'volume':
						$product = wc_get_product( $product_id );
						$volume = ( $product && ( $length = $product->get_length() ) && ( $width = $product->get_width() ) && ( $height = $product->get_height() ) ? $length * $width * $height : 0 );
						$cart_item_quantities[ $product_id ] = $item['quantity'] * $volume;
						break;
					case 'orders':
						$cart_item_quantities[ $product_id ] = 1;
						break;
				}
			}
